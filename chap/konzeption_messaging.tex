...
Aufgrund der genannten Limitierungen lehnt sich das Entwurfskonzept für D3OS an das Messaging-Modell der L4-Mikrokernarchitektur bzw. des verifizierten Mikrokerns seL4~\cite{elphinstone2016l4} an. Dieses Modell zeichnet sich durch drei Kernprinzipien aus:
...
\begin{tikzpicture}[scale=0.82, transform shape,
  ...
  \node[stepbox] (reg1) {Register\\(Sender)};
  \node[stepbox, right=2.3cm of reg1] (stack) {Stack\\(RAM)};
  \node[stepbox, right=2.3cm of stack] (tcb) {TCB\\(RAM)};
  \node[stepbox, right=2.3cm of tcb] (reg2) {Register\\(Empfänger)};

  \draw[flowarrow] (reg1) -- node[above=3pt, font=\scriptsize] {Syscall} (stack);
  \draw[flowarrow] (stack) -- node[above=3pt, font=\scriptsize] {Rust-Logik} (tcb);
  \draw[flowarrow] (tcb) -- node[above=3pt, font=\scriptsize] {Kontextwechsel} (reg2);
\end{tikzpicture}
\end{center}
